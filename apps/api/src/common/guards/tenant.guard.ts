...
export class TenantGuard implements CanActivate {
    canActivate(context: ExecutionContext): boolean {
        ...
        console.log('TenantGuard: user.tenantId=', user?.tenantId, ', headerTenantId=', headerTenantId);
        ...
        if (user && user.tenantId) {
            if (headerTenantId && headerTenantId !== user.tenantId) {
                console.log('TenantGuard: MISMATCH - Rejecting');
            ...
            if (!headerTenantId) {
                console.log('TenantGuard: MISSING HEADER - Rejecting');
                throw new ForbiddenException('x-tenant-id header is required');
            }
        }

        console.log('TenantGuard: PASSED');
        return true;
    }
}
